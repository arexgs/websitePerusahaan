...
function initCrop(imageSrc) {
    ...
    document.onmouseup = () => { cropState.isDragging = false; };
}
...
function updateCropTransform() {
    const img = document.getElementById('crop-img');
    if (img) img.style.transform = `scale(${cropState.scale}) translate(${cropState.x / cropState.scale}px, ${cropState.y / cropState.scale}px)`;
}
...
function applyCrop() {
    ...
    ctx.save();
    ctx.translate(100 + cropState.x, 100 + cropState.y);
